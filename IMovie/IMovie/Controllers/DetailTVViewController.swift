...
//  IMovie
//
//  Created by liza_kaganskaya on 4/15/19.
...
class DetailTVViewController: UIViewController {
    ...
    override func viewDidLoad() {
        super.viewDidLoad()
        trailersCollectionView.delegate = self as! UICollectionViewDelegate
        trailersCollectionView.dataSource  = self as! UICollectionViewDataSource
        
        self.perform(#selector(animateProgress), with: nil, afterDelay: 0.5)
        
        
        filmTitle.text = tv.name
        filmDesc.text = tv.overview
        releaseDate.text = ("Release Date: " + tv.first_air_date!)
    
        
        if let average = tv.vote_average {
            let rating = String(format:"%.1f", average)
            ratingLabel.text = "\(rating)"
        }
        
        
        if let bannerPath = tv.backdrop_path {
            DispatchQueue.main.async {
                self.filmImage.downloadImageFrom(urlString: bannerPath, posterSize:PosterSizes.BACK_DROP )
            }
        }
        
        
        TVMDB.videos(tvShowID: tvID!, language: "en") { (client, videosMD) in
